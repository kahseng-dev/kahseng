...
<div class="flex flex-col gap-4">
	...
	<div class="mt-12 grid grid-cols-2 gap-2">
		{#if projects[dataIndex - 1] != undefined}
			<a target="_self" href="/projects/{projects[dataIndex - 1]?.slug}" class="flex flex-col border border-(--border-0) transition hover:bg-white/10 rounded p-4">
				<span class="text-muted text-xs">last project</span>
				<span>{projects[dataIndex - 1]?.slug}</span>
			</a>
		{/if}
		{#if projects[dataIndex + 1] != undefined}
			<a target="_self" href="/projects/{projects[dataIndex + 1]?.slug}" class="col-start-2 flex flex-col border border-(--border-0) transition hover:bg-white/10 rounded p-4">
